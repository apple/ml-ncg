...
"""Routine for decoding the CIFAR-10/100 binary file format."""
...
def read_cifar(filename_queue):
    ...
    result.label = tf.cast(
        tf.strided_slice(record_bytes, [label_offset], [label_offset + label_bytes]), tf.int32)

    # The remaining bytes after the label represent the image, which we reshape
    # from [depth * height * width] to [depth, height, width].
    depth_major = tf.reshape(
        tf.strided_slice(record_bytes, [label_offset + label_bytes],
...
def distorted_inputs(data_dir, batch_size):
    ...
    Args:
      data_dir: Path to the CIFAR-10/100 data directory.
      batch_size: Number of images per batch.
    ...
    distorted_image = tf.image.resize_image_with_crop_or_pad(
        reshaped_image, height + 8, width + 8)
