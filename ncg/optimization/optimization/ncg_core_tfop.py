...
class NCG_OPTIMIZER_TFOP:
    ...
    Returns:
    """
    def __init__(self, g, precondition='Bfgs', line_search='Step', update_rule='FR', step_line_search_period=5, line_search_threshold=1, verbosity=0):
        ...
        self.h_diag = tf.get_variable(name='NCG_h_diag', shape=g_shape, dtype=tf.float32, initializer=tf.initializers.ones, regularizer=None, trainable=False, collections=[tf.GraphKeys.GLOBAL_VARIABLES])

        self.h_diag_eps = tf.get_variable(name='NCG_h_diag_eps', shape=g_shape,  dtype=tf.float32, initializer=tf.initializers.constant(1e-8), regularizer=None, trainable=False, collections=[tf.GraphKeys.GLOBAL_VARIABLES])

        self.g_old = tf.get_variable(name='NCG_g_old', shape=g_shape,  dtype=tf.float32, initializer=tf.initializers.zeros, regularizer=None, trainable=False, collections=[tf.GraphKeys.GLOBAL_VARIABLES])

        self.rtd_new = tf.get_variable(name='NCG_rtd_new', shape=[],  dtype=tf.float32, initializer=tf.initializers.zeros, regularizer=None, trainable=False, collections=[tf.GraphKeys.GLOBAL_VARIABLES])

        self.s = tf.get_variable(name='NCG_s', shape=g_shape,  dtype=tf.float32, initializer=tf.initializers.zeros, regularizer=None, trainable=False, collections=[tf.GraphKeys.GLOBAL_VARIABLES])

        self.d = tf.get_variable(name='NCG_d', shape=g_shape,  dtype=tf.float32, initializer=tf.initializers.zeros, regularizer=None, trainable=False, collections=[tf.GraphKeys.GLOBAL_VARIABLES])

        self.eps = tf.fill([], 1e-8, name='eps')

        self.base_lr = tf.get_variable(name='NCG_base_lr', shape=[],  dtype=tf.float32, initializer=tf.initializers.ones, regularizer=None, trainable=False, collections=[tf.GraphKeys.GLOBAL_VARIABLES])

        self.loss_ema = tf.get_variable(name='NCG_loss_ema', shape=[],  dtype=tf.float32, initializer=tf.initializers.zeros, regularizer=None, trainable=False, collections=[tf.GraphKeys.GLOBAL_VARIABLES])

        self.loss_ema_old = tf.get_variable(name='NCG_loss_ema_old', shape=[],  dtype=tf.float32, initializer=tf.initializers.zeros, regularizer=None, trainable=False, collections=[tf.GraphKeys.GLOBAL_VARIABLES])
    ...
    def online_line_search(self, lr, loss, old_loss, d, remove_old_dir=True):
        ...
        if self.verbosity > 0:
            loss_fn_ratio = tf.Print(loss_fn_ratio, [loss_fn_ratio, loss, old_loss], '[LOSS_FN_RATIO] [LOSS_FN] [LOSS_FN_OLD] :')
        ...
        d, base_lr = tf.cond(tf.less(loss_fn_ratio, tf.constant(1.0+0.01*self.line_search_threshold)),
                             ...
                             lambda: self.apply_line_search(lr, d, base_lr_degrade_ratio, old_dir_ratio, remove_old_dir))
    ...
    def get_direction(self, g, loss, lr, global_step):
        ...
        if global_step is None:
            self.global_step = tf.get_variable(name='NCG_global_step', shape=[],  dtype=tf.int32, initializer=tf.initializers.zeros, regularizer=None, trainable=False, collections=[tf.GraphKeys.GLOBAL_VARIABLES])
        ...
        if self.line_search == 'Online':
            ...
            print('USING STEP LINE SEARCH WITH THRESHOLD: ', self.line_search_threshold, 'AND FN COMPARISON PERIOD', self.step_line_search_period)
